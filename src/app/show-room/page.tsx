...
import { FetchCarListParams } from '@/entities/car/types';
import { CarList } from '@/features/car-list/ui/car-list';
import { FilterPanel } from '@/features/filter-panel';
import { Pagination } from '@/features/pagination';
//import { ShowRoomPanel } from '@/features/filter-panel/ui/filter-panel';
//import { Pagination } from '@/features/pagination';
import { Header } from '@/widgets/header';

type ShowRoomPageProps = {
  searchParams: { _limit: string; _page: string; _sort: string; _order: string };
};

export default async function ShowRoomPage({ searchParams }: ShowRoomPageProps) {
  const { _limit = itemsPerPage, _page = '1', _sort = 'price', _order = 'asc' } = await searchParams;

  const { data: cars, meta } = await fetchCarList({
    _limit: Number(_limit),
    _page: Number(_page),
    _sort,
    _order: _order as FetchCarListParams['_order'],
  });
